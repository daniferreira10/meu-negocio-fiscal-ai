...
const Hero = () => {
  ...
  return <section className="bg-gradient-to-br from-brand-dark-blue to-brand-dark py-16 md:py-24 text-white relative overflow-hidden">
      {/* Partículas de fundo */}
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(20)].map((_, i) => <motion.div key={i} className="absolute bg-brand-cyan rounded-full opacity-10" style={{
      ...
      <svg className="absolute inset-0 w-full h-full opacity-5" xmlns="http://www.w3.org/2000/svg">
        <motion.path d="M0,100 L200,100 C230,100 230,150 260,150 L500,150" stroke="#0ADBDF" strokeWidth="1" fill="none" initial={{
        pathLength: 0
      }} animate={{
        pathLength: 1
      }} transition={{
        ...
        <motion.path d="M100,0 L100,200 C100,230 150,230 150,260 L150,500" stroke="#0ADBDF" strokeWidth="1" fill="none" initial={{
        pathLength: 0
      }} animate={{
        pathLength: 1
      }} transition={{
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex flex-col md:flex-row items-center justify-between object-cover mx-[10px]">
          <div className="md:w-1/2 mb-12 md:mb-0">
            <motion.div variants={itemVariants} className="flex items-center mb-4">
              ...
              <motion.span className="bg-gradient-brand px-4 py-1 rounded-full text-xs font-semibold" whileHover={{
              scale: 1.05
            }}>
            ...
            <motion.h1 variants={itemVariants} className="text-4xl mb-6 leading-tight md:text-4xl font-semibold">
            ...
            {/* Stats Counter */}
            <motion.div variants={itemVariants} className="mt-10 grid grid-cols-3 gap-4 px-4 py-3 bg-brand-dark-blue/50 rounded-lg border border-brand-dark-blue/80 backdrop-blur-sm">
              {stats.map((stat, index) => <div key={index} className="text-center">
                  <motion.div className="text-xl md:text-2xl font-bold text-brand-cyan" initial={{
                opacity: 0,
                scale: 0.5
              }} animate={{
                opacity: 1,
                ...
                type: "spring"
                  ...
                  <motion.div className="text-xs text-gray-400" initial={{
                opacity: 0
              }} animate={{
                opacity: 1
              }} transition={{
          ...
          <motion.div variants={itemVariants} className="md:w-1/2 flex justify-center">
            <div className="relative">
              <motion.div className="absolute inset-0 bg-gradient-brand blur-2xl opacity-20 rounded-full" animate={{
              scale: [1, 1.05, 1],
              opacity: [0.2, 0.3, 0.2]
            }} transition={{
              duration: 4,
